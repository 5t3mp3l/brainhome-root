...
curl -sO http://192.168.188.193/remmina-setup-ct113.sh 2>/dev/null \
  || scp root@192.168.188.248:/home/workstation/scripts/remmina-setup-ct113.sh /tmp/
bash /tmp/remmina-setup-ct113.sh
```

Oder manuell (Script liegt auf proxmox-master):
```bash
scp root@192.168.188.248:/home/workstation/scripts/remmina-setup-ct113.sh /tmp/remmina-setup-ct113.sh
bash /tmp/remmina-setup-ct113.sh
```
...
## Lokale Benutzer automatisch für Nextcloud vorbereiten

Auf VM 113 reagiert `brainhome-user-cloud-provision.path` auf Änderungen an
`/etc/passwd`. Nach `adduser` werden alle regulären lokalen Benutzer mit einem
Home-Verzeichnis unter `/home` automatisch vorbereitet.

Der Ablauf installiert die aktuelle Caddy-Root-CA als vertrauenswürdige CA im
persönlichen Chrome/NSS-Speicher und erstellt den Nextcloud-Autostart. Die
Anmeldung am persönlichen Nextcloud-Konto und die Auswahl der Sync-Ordner
bleiben ein einmaliger interaktiver Schritt des Benutzers.

| Komponente | Pfad auf VM 113 |
| --- | --- |
| Einzelnen Benutzer konfigurieren | `/usr/local/sbin/configure-user-cloud-access.sh <benutzer>` |
| Alle lokalen Benutzer prüfen | `/usr/local/sbin/provision-local-cloud-users.sh` |
| Auslöser | `brainhome-user-cloud-provision.path` |
| Einmaliger Dienst | `brainhome-user-cloud-provision.service` |

Nach einer Benutzeranlage kann der Status geprüft werden:

```bash
systemctl is-active brainhome-user-cloud-provision.path
journalctl -u brainhome-user-cloud-provision.service -n 30 --no-pager
```

Repository-Quellen:

- [configure-user-cloud-access.sh](../scripts/configure-user-cloud-access.sh)
- [provision-local-cloud-users.sh](../scripts/provision-local-cloud-users.sh)
- [brainhome-user-cloud-provision.service](../systemd/brainhome-user-cloud-provision.service)
- [brainhome-user-cloud-provision.path](../systemd/brainhome-user-cloud-provision.path)
